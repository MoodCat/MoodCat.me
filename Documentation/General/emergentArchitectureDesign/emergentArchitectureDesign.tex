...
This makes retrieval of entities that use embeddables faster. For example we are most of the time not interested in the \class{VAVector} of a \class{Room}, only when we need to update such a \class{Room}, which happens rarely.
...
The \class{DBModule} uses a JpaPersistModule which is injected by Guice, whereas the \class{SoundCloudAPIConnector} uses a RestEasyClient.

\item Memento (JPA, Guice):\\
All database updates and retrievals are transactions, which impelement the Memento design pattern.
This makes sure that all database changes are ACID.
The methods are annotated by @Transactional, which Guice injects the transaction rollbacks.
Then JPA can perform the fail-safes whenever necessary.
...
\begin{enumerate}
\item Singleton:\\
In order to enforce a single controlling unit, all of our backends are @Singleton.
...
\item Observer:\\
We will use the Observer pattern after converting our currently pull-based architecture to a stream pipeline using websockets.
This way the frontend can continously receive updates whenever the backend notifies them.
Therefore chat messages can be instantly received and no unncessary calls shall be made to the frontend.
\end{enumerate}