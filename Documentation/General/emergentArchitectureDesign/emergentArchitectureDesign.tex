...
The mission is clear: Find a way for the system to scale further.
We went over several iterations of the system.
At first, we had the nearest neighbours algorithm.
Once we realised the issue here, we went for an ANN\footnote{Approximate nearest neighbour} algorithm to be able to use the nearest neighbour algorithm.
This initially was planned to be a \gls{KD-Tree}.
This still had issues with memory usage (O(N) complexity).
The second suggestion was grabbing a random subset of songs to run the NN algorithm on.
